...
def main():
    ...
    for split in SPLITS:
        ...
        for idx, example in enumerate(dataset[split]):
            ...
            metrics = evaluate_binary_classification(
                np.array(speech), np.array(vad_probs) > VAD_THRESHOLD
            )

            confident_metrics = evaluate_binary_classification(
                confident_speech[-1], confident_vad_probs[-1] > VAD_THRESHOLD
            )
        ...
        results[f"{split}_count"] = len(np.concatenate(all_speech))
        results[f"{split}_confidence_count"] = len(np.concatenate(confident_speech))
    ...
    for split in SPLITS:
        print(
            f"{split} has {((results[f'{split}_count'] * 512 / (16000 * 3600))):.2f} "
            f"hours of active speech with "
            f"{(results[f'{split}_confidence_count'] / results[f'{split}_count']):.2%} "
            "labelled confidence 1"
        )
